  ...
  node(label) {
    ...
    stage('Download .jar from S3') {
          withAWS(region: 'eu-central-1', credentials: 'aws-code-ro-access') {
           s3Download(file:'app.jar', bucket:"${CODE_REPO}", path:"release/com/surf/advisor/dict/application/${version}/application-${version}.jar", force:true)
          }
          sh 'chmod +x app.jar'
          sh 'ls -al'
    }
